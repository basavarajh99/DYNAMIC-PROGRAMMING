//similar to count paths
//Recursion TC: O(2^n*m) SC: O(m-1+n-1) i.e. path length
f(i, j) //we start from last cell n-1, m-1
{
  if(i>=0 and j>=0 and grid[i][j]==-1) return 0; //dead cell or obstacle in grid
...
f(i, j) //we start from last cell n-1, m-1
{
 ...
 for(int i=0; i<row; i++)
 {
  for(int j=0; j<col; j++)
  {
    if(grid[i][j]==-1) dp[i][j] = 0; //dead cell or obstacle in grid
...
f(i, j) //we start from last cell n-1, m-1
{
 ...
 for(int i=0; i<row; i++)
 {
  ...
  for(int j=0; j<col; j++)
  {
    if(grid[i][j]==-1) cur_row[j] = 0; //dead cell or obstacle in grid
